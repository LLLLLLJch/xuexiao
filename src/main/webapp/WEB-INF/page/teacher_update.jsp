	...
	<div style="width:400px; margin-left:50px">
		<form method="post"
		  ...
		  <table>
		  	...
		  	<tr>
		  		<td>
		  			...
		  			<div class="input-group date form_date col-md-5" data-date="" data-date-format="" data-link-field="dtp_input2" data-link-format="yyyy-MM-dd">
		               <input id="birthday" name="birthday" class="form-control" size="16" type="text" value="<fmt:formatDate value='${teacher.birthday}' pattern='yyyy-MM-dd'/>" readonly>
		  		...
		  		<td>
				    ...
				    <select class="form-control" name="coursename" id="course">
				    	<option value="${course.sequenceNO}">${course.name}</option>
				    	<c:forEach items="${courses}" var="cours">
					    	<option value="${cours.sequenceNO}">${cours.name}</option>
				    	</c:forEach>
				    </select>
		  		</td>
		  	</tr>
		  	<tr>
		  		<td>
		  			<img onclick="updatePicture();" alt="" src="http://owyysts4w.bkt.clouddn.com/${teacher.picture}" name=""  width="150" height="130">
		  			<input id="pictureInput" value="${teacher.picture}" type="hidden" name="picture"/>
...
<script>
	...
	function update(){
		var options = {
			url:"${pageContext.request.contextPath}/teacherIndex/updateTeacher.action",
			type:"post",
			dataType:"json",
