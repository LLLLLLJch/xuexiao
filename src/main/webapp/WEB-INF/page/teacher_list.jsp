...
<script>
	...
	function goDeleteTeacher(ID){
		layer.alert('',{
			icon:2,title:'删除确认',content:'您确定要删除这条记录吗？',closeBtn:1},function(index){
				$.post(
	            "${pageContext.request.contextPath}/teacherIndex/deleteTeacher.action",
	            {"ID":ID},
	            function(data) {
	            	if(data.status == 0){
	...
	function update(ID){
		layer.open({
  	      ...
  	      area: ['893px', '500px'],
  	      content: "${pageContext.request.contextPath}/teacherIndex/findUpdateTeacher.action?ID="+ID
  	    });
	}
	...
	function removeAll(){
		$("#searchForm")[0].reset(); 
		window.location.href="${pageContext.request.contextPath}/teacherIndex/findAllTeacher.action";
	}
	...
	function showTeacher(ID){
		layer.open({
	  	      ...
	  	      maxmin: true, //开启最大化最小化按钮
	  	      area: ['893px', '500px'],
	  	      content: "${pageContext.request.contextPath}/teacherIndex/showTeacher.action?ID="+ID
	  	    });
	}
</script>
</head>
<body>
	<form class="form-inline" action="${pageContext.request.contextPath}/teacherIndex/findTeacherByCondition.action" method="post" id="searchForm">
	...
	<input type="button" value="批量删除" onclick="deleteAll();" class="btn btn-default"/>
		<table class="table table-bordered">
			...
			<tr>
				<td>
					<input type="checkbox" name="selectedIds" value="${teacher.ID}">
				</td>
				<td><a href="javascript:showTeacher('${teacher.ID}');">${teacher.name}</a></td>
				<td>${teacher.age}</td>
				<td>
				...
				<td><fmt:formatDate value='${teacher.birthday}' pattern='yyyy-MM-dd'/></td>
				<td>${teacher.category.title}</td>
				<td><a href="javascript:update('${teacher.ID}')">修改</a></td>
				<td><a href="javascript:goDeleteTeacher('${teacher.ID}');">删除</a></td>
			</tr>
			</c:forEach>
		</table>
	</form>
